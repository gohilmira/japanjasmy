	...
	private function transfer($to, $amount)
		...
		public function auto_withdrawal()
		{
			...
			$datetime = date('Y-m-d');
			$all_cr_array = $this->conn->runQuery('*', 'users', "active_status=1 and id=2");
...
// 			echo 	$userid = 2;
				...
				echo "<br>final address--" .	$address = $user_details->user_address;
				echo "<br>final wallet_amt--" .	$wallet_amt;
				echo "<br>final _wd_limit--" .	$min_wd_limit;




				if ($wallet_amt >= $min_wd_limit) {
