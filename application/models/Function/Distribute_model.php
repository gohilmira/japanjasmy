...
class Distribute_model extends CI_Model
{
    ...
    function direct_roi_destribute($u_code, $amount, $no_of_levels = 10)
    {
        ...
        if ($plan) {
            ...
            while ($code != '') {
                ...
                $active_directs = $this->conn->runQuery('SUM(c9) as directs', 'user_wallets', "u_code='$code' ")[0]->directs;
                ...
                if ($payable > 0 && $code != '') {
                    ...
                    $income['tx_record'] = $l;
                    $income['remark'] = "Recieve Leval Income of $ $payable from $name ($username)  level $l ";
    ...
    function level_destribute($u_code, $amount, $no_of_levels = 5)
    {
        $code = $u_code;
        $ben_from = $u_code;
        $incomes = array();
        ...
        if ($plan) {
            ...
            while ($code != '') {
                ...
                if ($level_distribution_in == 'fix') {
                    ...
                    $payable_amt1 = $level_per;
                } else {
                    $level_per = $level_income[$l];
                    $payable_amt1 = $amount * $level_per / 100;
                }
               ...
               // echo "dgdfff"; die;
                ...
                if ($payable_amt1 > 0 && $code != '') {
                    ...
                    $income['tx_record'] = $l;
                    $income['remark'] = "Recieve $source income of amount $payable_amt1 from $name ($username) from level $l";
                    if ($this->db->insert('transaction', $income)) {
    ...
    function direct_destribute($u_code, $amount, $no_of_levels = 1)
    {
        $code = $u_code;
        $ben_from = $u_code;
        $incomes = array();
        ...
        if ($plan) {
            $direct_income = array_column($plan, 'direct_income', 'id');
            while ($code != '') {
                $source = 'direct';
                $currenct_payout_id = $this->wallet->currenct_payout_id();
                $level_distribution_in = $this->conn->setting('level_distribution_in');
                if ($level_distribution_in == 'fix') {
                    $level_per = $direct_income[$l];
                    $payable_amt1 = $level_per;
                } else {
                    $level_per = $direct_income[$l];
                    $payable_amt1 = $amount * $level_per / 100;
               ...
               // echo "dgdfff"; die;
                ...
                if ($payable_amt1 > 0 && $code != '') {
                    ...
                    $income['wallet_type'] = 'main_wallet';
                    $income['tx_record'] = $l;
                    $income['remark'] = "Recieve $source income of amount $payable_amt1 from $name ($username) from Direct ";
                    if ($this->db->insert('transaction', $income)) {
